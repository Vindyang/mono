...
export default async function TaskDetailsPage({
  ...
  const { task, project, projects, currentUserRole, error } = await getTaskById(
    resolvedParams.id,
  );
  ...
  return (
    <div className="flex flex-col gap-6 w-full p-4 md:p-6 max-w-7xl mx-auto">
      ...
      <div className="max-w-4xl mx-auto space-y-8">
          ...
          <div className="flex flex-wrap items-center gap-4 p-4 bg-card border border-border rounded-xl shadow-sm">
            ...
            {project && (
              <>
                ...
                <Link
                  href={`/projects/${project.id}`}
                  className="flex items-center gap-1.5 text-sm text-foreground hover:text-primary transition-colors font-medium ml-auto"
                >
            ...
            <div className="bg-card border border-border rounded-xl p-6 shadow-sm">
              ...
              <div className="flex flex-wrap gap-3">
                {task.assignees.map((assignee) => (
                  <div
                    key={assignee.id}
                    className="flex items-center gap-3 bg-secondary/50 rounded-lg px-4 py-2.5 hover:bg-secondary transition-colors"
                  >
        ...
        {/* Attachments Section */}
        {(task.image || (task.attachments && task.attachments.length > 0)) && (
