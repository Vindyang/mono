...
import {
  ...
  deleteTask,
  updateTaskStatus,
} from "@/app/(authenticated)/tasks/componentsaction/actions";
...
export default function ProjectDetailsPage({
  ...
  useEffect(() => {
    ...
    const fetchProjectData = async () => {
      try {
        setIsLoading(true);
        const { project: projectData, error } = await getProjectDetails(
          resolvedParams.id,
        );
  ...
  const refreshProjectData = async () => {
    const { project: updatedProject, error: fetchError } =
      await getProjectDetails(resolvedParams.id);
  ...
  const handleCreateTask = async (
    ...
    try {
      const { success, task, error } = await createTask({
        ...
        pendingAttachments: taskData.pendingAttachments,
      });
  ...
  const canEditTasks =
    project.currentUserRole === "OWNER" || project.currentUserRole === "LEAD";
    ...
    <div className="flex flex-col min-h-[calc(100vh-6rem)] gap-4 md:gap-6 p-4 md:p-6">
      ...
      <div>
        <Button
          variant="ghost"
          asChild
          className="pl-0 hover:bg-transparent hover:text-foreground text-muted-foreground group"
        >
        ...
        <div>
          ...
          {project.description && (
            <div className="mt-1 w-full">
              <p
                className={`text-lg text-muted-foreground ${!isDescriptionExpanded ? "line-clamp-2 mb-1" : ""}`}
              >
                {project.description}
              </p>
              ...
              <div className="flex items-center gap-2 cursor-pointer hover:opacity-80 transition-opacity">
                <div className="flex -space-x-2">
                  {project.members.slice(0, 5).map((member) => (
                    <Avatar
                      key={member.id}
                      className="w-6 h-6 border-2 border-background grayscale hover:grayscale-0 transition-all"
                    >
              ...
              {project.tasks.length > 0 ? (
                project.tasks.map((task) => (
                  <TaskCard
                    ...
                    onEdit={
                      canEditTasks ? () => openEditModal(task) : undefined
                    }
                    onDelete={
                      canEditTasks ? () => handleDeleteTask(task) : undefined
                    }
                    onStatusToggle={() =>
      ...
      <TaskModal
        key={editingTask ? `edit-${editingTask.id}` : "create-project-task"}
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              from the project. They will lose access to all project tasks.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
